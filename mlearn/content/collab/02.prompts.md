...
During a session, chatGPT remembers previous prompts, which makes chatGPT open to exploitation.

> a chatGPT prompt modifies the way AI would normally respond, usually by specifying output formats and protocols for interaction.  Malicious prompts can be used to trick chatGPT into misbehavior, making it ignore its owns ethical and legal guidelines.
...
PAT stands for "Philosophy and Thinking”, here's the prompt
...
Nice to have that conversation. It made me think about the ethics of sharing malicious prompts.  :icon{name="emojione:anxious-face-with-sweat"}
...
Just run COMPARE "some text", and chatGPT will keep up the charade, and maybe wake up after a few prompts, or refuse to keep playing.
